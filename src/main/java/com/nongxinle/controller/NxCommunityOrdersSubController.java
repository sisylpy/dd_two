...
public class NxCommunityOrdersSubController {
    ...
    @ResponseBody
    public R testPrint(Integer countValue) {
        ...
        if(subEntities.size() > countValue){
            for(int i = 0; i < countValue; i++){
                NxCommunityOrdersSubEntity subEntity = subEntities.get(i);
                subEntity.setNxCosStatus(1);
                System.out.println("tesrtpriririr" + subEntity.getNxCosStatus() +"id=" + subEntity.getNxCommunityOrdersSubId());
    ...
    @RequestMapping(value = "/getUnPrintSubOrders0", method = RequestMethod.POST)
    @ResponseBody
    public R getUnPrintSubOrders0 (Integer commId, Integer status) {
